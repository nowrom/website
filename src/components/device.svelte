...
<div class="min-h-screen mb-10">
  ...
  <div
    class="flex justify-center max-w-[80rem] mx-auto gap-4 flex-col md:flex-row"
  >
    <div>
    ...
    <div class="w-full flex flex-col gap-2">
      <div class="bg-primary/30 p-4 rounded-md w-full">
        <article>
          ...
          <p class="text-white">
            Custom roms for the <strong class="text-green-400"
              >{device.name}</strong
            >: {device.roms
              .map((rom) => {
              ...
              .filter((x) => x)
              .join(", ")}.<br />
            You can view the list down below to download these roms and find more
      ...
      <div
        class="bg-primary/30 p-4 rounded-md w-full grid gap-2 text-neutral-300"
      >
        <h2 class="text-3xl text-slate-200">Recoveries</h2>
        <p>
          Recoveries that officially support the {device.name} if theres no official
          recovery for the
